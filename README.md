...
1. MATLAB scripts for generating theoretical magnetic maps and crowdsourced multi-trajectory smartphone observations.
2. MATLAB scripts for projecting trajectory measurements onto a common grid and producing fused magnetic observation maps.
3. A Python/PyTorch example that loads a fused test map from the Kursk validation setting, applies the trained GSmartMagDL model, and saves the reconstructed magnetic map.
4. A theoretical reference map for the same test case, used only for visual comparison and evaluation.

In the Python example, the input file `Bsim_tot_1.mat` represents the fused observation map generated by the MATLAB pipeline, and `Btheor_tot_1.mat` provides the corresponding theoretical magnetic reference.
